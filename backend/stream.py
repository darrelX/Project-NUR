...
def main():
    ...
    # Bannière d'aide
    with st.expander("📖 Guide d'utilisation - LISEZ CECI EN CAS D'ERREUR", expanded=False):
        st.markdown("""
        ### 🎯 Ordre des Étapes (TRÈS IMPORTANT)
        
        **Pour éviter les erreurs "colonne introuvable" ou "hors limites" :**
        
        1. **📁 D'ABORD** : Uploadez le fichier source en haut
        2. **⚙️ ENSUITE** : Pour chaque opération :
           - ✅ Cochez "Activer [Opération]"
           - 📤 **Uploadez/entrez le fichier cible AVANT de toucher aux dropdowns**
           - ⏳ Attendez 2-3 secondes que les dropdowns se remplissent
           - 📋 Sélectionnez les colonnes dans les dropdowns (ne tapez PAS manuellement)
        3. **▶️ ENFIN** : Cliquez sur "Exécuter"
        
        ### ⚠️ Erreurs Courantes
        
        - **"Colonne X hors limites"** → Vous avez configuré les colonnes AVANT d'uploader le fichier cible
          - **Solution** : Rechargez la page (F5) et recommencez dans le bon ordre
        
        - **Dropdowns vides** → Le fichier cible n'est pas chargé
          - **Solution** : Uploadez d'abord le fichier cible, puis les dropdowns apparaîtront
        
        - **Valeurs incorrectes** → Vous avez tapé manuellement au lieu d'utiliser les dropdowns
          - **Solution** : Utilisez UNIQUEMENT les dropdowns une fois le fichier chargé
        
        ### 💡 Astuce
        Si les dropdowns ne se remplissent pas, **rechargez la page (F5)** et recommencez.
        """)
    ...
    # =============================================================================
    st.header("📁 Fichier Source Commun")
    ...
    if source_file and Path(source_file).exists():
        with st.expander("👁️ Aperçu du fichier source"):
            try:
                df_preview = pd.read_excel(source_file, nrows=10)
                st.dataframe(df_preview, use_container_width=True)
                # Convertir les noms de colonnes en strings
                col_names_preview = [str(col) for col in df_preview.columns]
                st.info(f"📊 Colonnes disponibles: {', '.join(col_names_preview)}")
            except Exception as e:
    ...
    st.info("💡 **Workflow recommandé**: 1️⃣ Uploadez d'abord le fichier cible → 2️⃣ Activez l'opération → 3️⃣ Les dropdowns se remplissent automatiquement avec les colonnes disponibles")
    ...
    with tab1:
        ...
        if cd_enabled:
            st.warning("⚠️ **IMPORTANT**: Uploadez/sélectionnez d'abord le fichier cible ci-dessous, PUIS configurez les colonnes. Sinon les dropdowns ne se rempliront pas correctement.")
            ...
            # Avertissement si pas de fichier cible
            if not cd_target_file:
                st.warning("⚠️ Aucun fichier cible sélectionné. Uploadez un fichier ou entrez un chemin manuel.")
            elif not Path(cd_target_file).exists():
                st.error(f"❌ Fichier cible introuvable: {cd_target_file}")
            ...
            with col1:
                ...
                if source_sheets:
                    ...
                    cd_source_sheet = st.text_input(
                        ...
                        value="Sheet1",
                        key="cd_source_sheet_manual"
                    )
                
                # Colonne clé dans source (avec dropdown)
                cd_source_key = column_selector(
                    "Colonne clé dans source",
                    source_file,
    ...
    with tab2:
        ...
        if sx_enabled:
            ...
            # Avertissement si pas de fichier cible
            if not sx_target_file:
                st.warning("⚠️ Aucun fichier cible sélectionné. Uploadez un fichier ou entrez un chemin manuel.")
            elif not Path(sx_target_file).exists():
                st.error(f"❌ Fichier cible introuvable: {sx_target_file}")
            ...
            with col1:
                ...
                if target_sheets:
                    ...
                    sx_target_sheet = st.text_input(
                        "Feuille cible",
                        value="Sheet1",
                        key="sx_target_sheet_manual"
                    )
                
                # Colonne clé source (avec dropdown)
                sx_source_key = column_selector(
                    "Colonne clé source",
                    source_file,
    ...
    with tab3:
        ...
        if tk_enabled:
            ...
            # Avertissement si pas de fichier cible
            if not tk_target_file:
                st.warning("⚠️ Aucun fichier cible sélectionné. Uploadez un fichier ou entrez un chemin manuel.")
            elif not Path(tk_target_file).exists():
                st.error(f"❌ Fichier cible introuvable: {tk_target_file}")
            ...
            with col2:
                ...
                if target_columns:
                    ...
                    tk_join_cols = ",".join(tk_join_cols_list) if tk_join_cols_list else "A,B"
                else:
    ...
    if execute_button:
        if not source_file or not Path(source_file).exists():
            ...
            # Vérifier que les fichiers cibles existent pour les opérations activées
            errors = []
            if cd_enabled and (not cd_target_file or not Path(cd_target_file).exists()):
                errors.append("CellDown: Fichier cible manquant ou invalide")
            if sx_enabled and (not sx_target_file or not Path(sx_target_file).exists()):
                errors.append("SuperXlookup: Fichier cible manquant ou invalide")
            if tk_enabled and (not tk_target_file or not Path(tk_target_file).exists()):
                errors.append("Ticket: Fichier cible manquant ou invalide")
            
            if errors:
                st.error("❌ Erreurs de configuration détectées:")
                for err in errors:
                    st.error(f"  • {err}")
                st.warning("💡 Assurez-vous d'avoir uploadé ou saisi le chemin de tous les fichiers cibles pour les opérations activées.")
                st.info("🔄 **Conseil** : Si vous avez des erreurs de colonnes, rechargez la page (F5) et recommencez en uploadant d'ABORD les fichiers cibles.")
                st.stop()
            
            # Vérifier que les dropdowns ont été remplis correctement
            warnings = []
            if cd_enabled:
                # Vérifier si les colonnes sont des lettres par défaut alors que le fichier est chargé
                if cd_target_file and Path(cd_target_file).exists():
                    try:
                        df_check = pd.read_excel(cd_target_file, nrows=0)
                        cols = [str(c) for c in df_check.columns]
                        if cd_target_key == "A" and "A" not in cols and len(cols) > 0:
                            warnings.append(f"CellDown: La colonne clé cible est 'A' (valeur par défaut). Colonnes réelles : {', '.join(cols[:5])}")
                        if cd_target_value == "A" and "A" not in cols and len(cols) > 0:
                            warnings.append(f"CellDown: La colonne valeur est 'A' (valeur par défaut). Colonnes réelles : {', '.join(cols[:5])}")
                    except:
                        pass
            
            if sx_enabled:
                if sx_target_file and Path(sx_target_file).exists():
                    try:
                        df_check = pd.read_excel(sx_target_file, nrows=0)
                        cols = [str(c) for c in df_check.columns]
                        if sx_target_key == "A" and "A" not in cols and len(cols) > 0:
                            warnings.append(f"SuperXlookup: La colonne clé cible est 'A' (valeur par défaut). Colonnes réelles : {', '.join(cols[:5])}")
                        if sx_target_value == "A" and "A" not in cols and len(cols) > 0:
                            warnings.append(f"SuperXlookup: La colonne valeur est 'A' (valeur par défaut). Colonnes réelles : {', '.join(cols[:5])}")
                    except:
                        pass
            
            if tk_enabled:
                if tk_target_file and Path(tk_target_file).exists():
                    try:
                        df_check = pd.read_excel(tk_target_file, nrows=0)
                        cols = [str(c) for c in df_check.columns]
                        if tk_extract_col == "A" and "A" not in cols and len(cols) > 0:
                            warnings.append(f"Ticket: La colonne extraction est 'A' (valeur par défaut). Colonnes réelles : {', '.join(cols[:5])}")
                    except:
                        pass
            
            if warnings:
                st.warning("⚠️ Attention - Colonnes suspectes détectées:")
                for warn in warnings:
                    st.warning(f"  • {warn}")
                st.error("🛑 Vous utilisez probablement des valeurs par défaut au lieu des vraies colonnes de vos fichiers.")
                st.info("🔄 **Solution** : Rechargez la page (F5), uploadez d'ABORD les fichiers cibles, PUIS sélectionnez les colonnes dans les dropdowns.")
                
                if not st.button("⚠️ Ignorer et continuer quand même (RISQUÉ)", type="secondary"):
                    st.stop()
        ...
        if source_file and Path(source_file).exists():
            try:
                ...
                with st.expander("📋 Liste complète des colonnes"):
                    # Convertir les noms de colonnes en strings
                    cols_display = ", ".join([f"`{str(col)}`" for col in df_final.columns])
...
def execute_operations(source_file, cd_enabled, sx_enabled, tk_enabled, cd_params, sx_params, tk_params):
    ...
    try:
        ...
        col_names = [str(col) for col in df_before.columns]
        st.session_state.execution_log.append({
            'operation': 'ÉTAT INITIAL',
            'status': 'info',
            'message': f'Fichier source avec {cols_before} colonnes: {", ".join(col_names)}'
        })
    except Exception as e:
        st.session_state.execution_log.append({
            'operation': 'ÉTAT INITIAL',
    ...
    if cd_enabled:
        try:
            ...
            error_msg = str(e)
            # Ajouter info sur colonnes disponibles si erreur de colonne
            if 'hors limites' in error_msg or 'introuvable' in error_msg.lower():
                try:
                    df_check = pd.read_excel(cd_params['target_file'], nrows=0)
                    cols = [str(c) for c in df_check.columns]
                    error_msg += f"\n\n📋 Colonnes disponibles dans le fichier cible: {', '.join(cols)}"
                except:
                    pass
            ...
            st.session_state.execution_log.append({
                'operation': 'CellDown',
                'status': 'error',
                'message': f'Erreur: {error_msg}\n\nℹ️ Astuce: Rechargez la page et uploadez/sélectionnez d\'abord le fichier cible avant de configurer les colonnes.'
            })
    ...
    if sx_enabled:
        try:
            ...
            if 'hors limites' in error_msg or 'introuvable' in error_msg.lower():
                try:
                    df_check = pd.read_excel(sx_params['target_file'], nrows=0)
                    cols = [str(c) for c in df_check.columns]
                    error_msg += f"\n\n📋 Colonnes disponibles dans le fichier cible: {', '.join(cols)}"
                except:
                    pass
    ...
    if tk_enabled:
        try:
            ...
            if 'hors limites' in error_msg or 'introuvable' in error_msg.lower():
                try:
                    df_check = pd.read_excel(tk_params['target_file'], nrows=0)
                    cols = [str(c) for c in df_check.columns]
                    error_msg += f"\n\n📋 Colonnes disponibles dans le fichier cible: {', '.join(cols)}"
                except:
                    pass
    ...
    try:
        ...
        # Convertir tous les noms de colonnes en strings
        col_names_after = [str(col) for col in df_after.columns]
        
        st.session_state.execution_log.append({
            'operation': 'ÉTAT FINAL',
            'status': 'info',
            'message': f'Fichier final avec {cols_after} colonnes (+{cols_added} nouvelles): {", ".join(col_names_after)}'
        })
    except Exception as e:
        st.session_state.execution_log.append({
            'operation': 'ÉTAT FINAL',
