...
class PersonnalizedXlookup:
    ...
    def run(self) -> None:
        ...
        if self.target_join_columns is not None and len(self.target_join_columns) > 0:
            join_cols_real = self._resolve_columns_list(df_target_ready, self.target_join_columns)
            if not join_cols_real:
                raise ValueError("❌ Aucune colonne valide trouvée dans target_join_columns")
        ...
        elif self.target_value_column is not None:
            ...
            # Si aucune colonne n'est spécifiée, on essaie d'utiliser la première colonne qui n'est pas la clé
            potential_cols = [col for col in df_target_ready.columns if col != target_key_col and not col.startswith('__')]
            if not potential_cols:
                raise ValueError("❌ Aucune colonne de valeur trouvée. Spécifiez 'target_value_column' ou 'target_join_columns'")
            value_col = potential_cols[0]
            print(f"⚠️ Aucune colonne spécifiée, utilisation automatique de : {value_col}")
        ...
        # ------------------------------------------------------------
        # CORRECTION : calcul de la position AVANT suppression
        # ------------------------------------------------------------
        ...
        if result_col_name in df_source.columns:
            ...
            # Ajuster l'index si la colonne supprimée était avant la position voulue ?
            # Non, car on veut insérer à result_idx (la position désirée).
            # Si la colonne supprimée était avant result_idx, les colonnes se décalent
            # mais result_idx reste valable car on veut insérer avant la colonne qui
            # était à l'original result_idx (ou à la fin).
            # On ne modifie pas result_idx.

        # Insérer la nouvelle colonne à la position calculée
        # Si result_idx est plus grand que le nombre de colonnes actuel, on ajoute à la fin
        if result_idx > len(df_source.columns):
            # Ajouter des colonnes vides si nécessaire (cas où on veut décaler)
            for i in range(result_idx - len(df_source.columns)):
                df_source[f"_tmp_{i}"] = ""
            # On insère à la fin (index = len(df_source.columns) après ajout des temporaires)
...
if __name__ == "__main__":
    personnalizedXlookup = PersonnalizedXlookup(
        source_file_path=r"C:\Users\f50056342\Desktop\computer science\NUR Project Lyne\inputs\Book1.xlsx",
        target_file_path=r"C:\Users\f50056342\Desktop\my work\Top Offenders\top offenders 14-06-2026_update.xlsx",
        source_key_column=["code site", "site code", "site id"],
        result_position_column="last_free",
        result_column_name="Top Offenders",
        source_sheet_name="Sheet1",
        target_sheet_name="ALL",
        reference_name="Top Offenders",
